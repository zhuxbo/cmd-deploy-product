...
backup_current() {
    ...
    # 记录当前版本
    if [ -f "$SITE_ROOT/VERSION" ]; then
        CURRENT_VERSION=$(cat "$SITE_ROOT/VERSION")
...
deploy_new_files() {
    ...
    case "$UPDATE_MODULE" in
        all)
            ...
            # 复制版本文件
            cp "$SOURCE_PATH/VERSION" "$SITE_ROOT/" 2>/dev/null || true
            cp "$SOURCE_PATH/BUILD_INFO.json" "$SITE_ROOT/" 2>/dev/null || true
            ;;
...
set_permissions() {
    ...
    for component in admin user easy; do
        if [ -d "$SITE_ROOT/frontend/$component" ] && ([ "$UPDATE_MODULE" = "all" ] || [ "$UPDATE_MODULE" = "$component" ]); then
            ...
            # 代码文件设置为644（只读）
            find "$SITE_ROOT/frontend/$component" -type f \( -name "*.html" -o -name "*.js" -o -name "*.css" -o -name "*.json" -o -name "*.svg" -o -name "*.png" -o -name "*.jpg" -o -name "*.jpeg" -o -name "*.gif" -o -name "*.ico" -o -name "*.xml" -o -name "*.txt" -o -name "*.md" \) -exec sudo chmod 644 {} \;
        fi
    done
